...
use profont::PROFONT_14_POINT; // a bit dark
//use u8g2_fonts::fonts::u8g2_font_10x20_tn;
...
pub struct SpiDriver<'d>{
    spi: Spi<'d, SPI0, embassy_rp::spi::Blocking>,
}
const DISPLAY_CONFIG : Config = spi::Config;

impl <'d, T : embassy_rp::spi::Instance> SpiDriver<'d>{
    pub fn new(spawner: Spawner, 
               spi: Peri<'d, impl PeripheralType>,
               sck: Peri<'d, impl ClkPin<T> + 'd>,
               mosi: Peri<'d, impl MosiPin<T> + 'd>,
               miso: Peri<'d, impl MisoPin<T> + 'd>,  
        ) -> Self {
            //let config = 
             let display_config = spi::Config::default();//Config::default();
            let spi = Spi::new_blocking(spi, sck, mosi, miso, DISPLAY_CONFIG);
            SpiDriver { spi}
        }
...
// struct Peripherals{
//     spi: Option<dyn SpiDevice<Error = SpiDeviceError<>>,

//     spi_bus: Option<Mutex<NoopRawMutex, _>,
...
//     // let mut display: st7565::ST7565<SPIInterface<SpiDeviceWithConfig<'_, NoopRawMutex, Spi<'_, SPI0, Blocking>, Output<'_>>, Output<'_>>, DOGL128_6, st7565::modes::GraphicsMode<'_, 128, 8>, 128, 64, 8> = st7565::ST7565::new(display_interface, DOGL128_6)
...
async fn main_(spawner: Spawner) {
    ...
    // let mut screen = screen::Screen::new(Screen<'_> , &ST7565<SPIInterface<embassy_embedded_hal::shared_bus::blocking::spi::SpiDeviceWithConfig<'a, NoopRawMutex, embassy_rp::spi::Spi<'_, embassy_rp::peripherals::SPI0, embassy_rp::spi::Blocking>, Output<'_>>, Output<'_>>, DOGL128_6, GraphicsMode<'_, 128, 8>, 128, 64, 8> );
    info!("Started");
    ...
    let spi = SpiDriver::new_blocking(spawner, spi, sclk, mosi, miso);
...
//     let mut page_buffer = GraphicsPageBuffer::new();
    ...
    // let font = MonoTextStyle::new(&main_font, BinaryColor::On);
    ...
    //         calc.stack.fill_y_to_t(0.0, 0.0, 0.0);
    ...
    //         // info!("x_buffer_str:");
    //         // for a in x_buffer_str.chars() {
    //         //     info!("{}",a as char);
    //         // }
    //         // info!("in main: x: {}, y: {}, z: {} t: {}", x_val, y_val, z_val, t_val);
